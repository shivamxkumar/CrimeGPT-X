  ...
  return (
    <AppShell>
      ...
      {tab === 'config' && (
        <div className="grid grid-cols-2 gap-5 animate-slide-in">
          ...
          <div className="card">
            ...
            <div className="text-xs text-text-muted">Email notifications are configured via SMTP_HOST/SMTP_USER environment variables on the backend. Deadline alerts run automatically every 6 hours via the scheduled Celery task.</div>
          </div>
        </div>
      )}
      ...
      {tab === 'system' && (
        <div className="grid grid-cols-2 gap-5 animate-slide-in">
          {statusLoading ? (
            <div className="col-span-2 flex justify-center py-16"><Spinner size="lg"/></div>
          ) : statusError ? (
